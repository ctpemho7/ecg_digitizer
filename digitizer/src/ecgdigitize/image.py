...
def scaleImage(inputImage: Image, gridSizeInPixels: int, parameters: InputParameters):
    timeScale = parameters.timeScale

    if gridSizeInPixels > 100:
        gridSizeInPixels = gridSizeInPixels / 5

    print("gridSizeInPixels for 1 mm", gridSizeInPixels)
    resize_coef_seconds = (timeScale / gridSizeInPixels)
    # resize_coef_seconds = 0.75
    print("resize_coef_seconds", resize_coef_seconds)

    height, width, _ = inputImage.data.shape
    new_size = (int(width * resize_coef_seconds), height)
    resized = cv2.resize(inputImage.data, new_size, interpolation=cv2.INTER_LINEAR)

    if isinstance(inputImage, ColorImage):
        return ColorImage(resized)
    elif isinstance(inputImage, GrayscaleImage):
        return GrayscaleImage(resized)
    elif isinstance(inputImage, BinaryImage):
        return BinaryImage(resized)
    else:
        raise ValueError
