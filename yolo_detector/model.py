...
class YOLOv8n(LabelStudioMLBase):
    """Pretrained YOLOv8n model for ECG lead detection"""
    ...
    def _get_image_url(self, task: Dict) -> str:
        ...
        if image_url.startswith("s3://") and os.getenv('AWS_ACCESS_KEY_ID'):
            ...
            try:
                ...
                print(f"Can't generate pre-signed URL for {image_url}. Reason: {exc}")
    ...
    def predict(self, tasks: List[Dict], context: Optional[Dict] = None, **kwargs) -> ModelResponse:
        ...
        model = YOLO(r"weights/best_y8n.pt")  # pretrained YOLOv8n model
        predictions = []
        for task in tasks:
            ...
            bboxes = result.boxes.xywhn

            results = []
            all_scores = []
            for i in range(len(bboxes)):
                x, y, width, height = [value.item() for value in result.boxes.xywhn[i]]
                confidence = result.boxes.conf[i].item()
                class_label = CLASS_MAP[result.boxes.cls[i]].item()
